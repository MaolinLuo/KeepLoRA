...
from videollava.train.train import make_supervised_data_module, DataArguments
...
def extract_gradients(args):
    ...
    model_base = model_config.get('model_name')
    model_name = get_model_name_from_path(model_path)
    if "merged" in model_path or model_path == model_base:
        model_base = None
    ...
    # important when using gradient checkpointing with many frozen params
    ...
    data_args = DataArguments(
        ...
        image_folder=None,
        video_folder=data_config['train_folder'],
    ...
    video_tower.to(dtype=torch.float16, device=device)
    data_args.video_processor = video_tower.video_processor
    data_args.num_frames = video_tower.config.num_frames
    # 8 -> 4
    default_num_frames = video_tower.config.num_frames
    num_frames = min(default_num_frames, 4)
    data_args.num_frames = num_frames
        ...
        with torch.no_grad():
            for name, param in model.named_parameters():
                if name in accumulated_grads and param.grad is not None:
                    ...
                    accumulated_grads[name].mul_((num_batches - 1) / num_batches)
                    accumulated_grads[name].add_(param.grad.detach(), alpha=1.0 / num_batches)
...
def merge_gradient_chunks(output_dir, num_chunks, train_config_path, svd_mode):
    """
    ...
    fixed_rank = train_config.get('rank')
    energy_threshold = train_config.get('energy_threshold')
    space_path = train_config.get('space_path')
...
if __name__ == "__main__":
    ...
    if args.merge_only:
        merge_gradient_chunks(args.output_dir, args.num_chunks, args.train_config, svd_mode=args.svd_mode)
    else:
