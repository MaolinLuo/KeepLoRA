...
from videollava.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, \
    ...
    MAX_VIDEO_LENGTH
from videollava.peft.utils import WEIGHTS_NAME, set_peft_model_state_dict
...
# Custom LoRA utilities for freezing A matrix and custom initialization
def freeze_lora_A_matrices(model, local_rank=-1):
    """
    Freeze all LoRA A matrices in the model.
    Compatible with DeepSpeed ZeRO.
    """
    from videollava.peft.tuners.lora import LoraLayer
...
def initialize_lora_weights_custom(model, gradient_dict, lora_r, local_rank=-1):
    ...
    if local_rank in [0, -1]:
        rank0_print(f"Successfully initialized {initialized_count} LoRA matrices with pre-computed SVD")
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    freeze_lora_A: bool = field(default=False, metadata={"help": "Freeze LoRA A matrices, only train B matrices"})
    init_lora_from_gradients: Optional[str] = field(default=None, metadata={"help": "Path to gradient file (.pt) for SVD-based LoRA initialization"})
...
class LazySupervisedDataset(Dataset):
    ...
    def __init__(self, data_path: str,
        ...
        if isinstance(data_path, str):
            data_path = [data_path] # list_data_dict = json.load(open(data_path, "r"))
...
def load_model_from_previous_task(model, previous_task_model_path):
    ...
    from videollava.peft import PeftModel
...
def train():
    ...
    if training_args.bits in [4, 8]:
        from videollava.peft import prepare_model_for_kbit_training
    ...
    if training_args.lora_enable:
        from videollava.peft import LoraConfig, get_peft_model
        ...
        # SVD-based initialization from pre-computed gradients
        if training_args.init_lora_from_gradients is not None:
            rank0_print(f"Loading pre-computed SVD results from {training_args.init_lora_from_gradients}...")
            gradient_data = torch.load(training_args.init_lora_from_gradients, map_location='cpu')
            gradient_dict = gradient_data['gradients']
            
            # Initialize A with pre-computed SVD results, B remains zero
            initialize_lora_weights_custom(
                model, 
        ...
        # Freeze LoRA A matrices if requested (before DeepSpeed wrapping)
        if training_args.freeze_lora_A:
            rank0_print("Freezing LoRA A matrices...")
            freeze_lora_A_matrices(model, local_rank=local_rank)
